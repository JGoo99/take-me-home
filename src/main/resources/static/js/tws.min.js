...
(function () {
...
})()(
  {
      ...
      function (require, module, exports) {
        ...
        tsw = (function () {
          ...
          var isTswNode = function (thing) {
            return (
              thing.hasOwnProperty("input") || thing.hasOwnProperty("output")
            );
          };
          ...
          var scheduleTransition = function (
            ...
            if (exists(transition_type)) {
              switch (transition_type) {
                case "linear":
                  node[param_to_change].setValueAtTime(
                    node[param_to_change].value,
                    tsw.now()
                  );
                  node[param_to_change].linearRampToValueAtTime(
                    param_value,
                    targetTime
                  );
                  break;
                case "exponential":
                  ...
                  node[param_to_change].exponentialRampToValueAtTime(
                    param_value,
                    targetTime
                  );
                  break;
          ...
          var checkBrowserSupport = function (success, failure) {
            ...
            if (
              ...
              if (typeof webkitAudioContext === "function") {
                if (
                  typeof webkitAudiocontext().prototype.createGainNode ===
                  "undefined"
                ) {
                  failure(
                    "Sorry, your browser doesn't support a recent enough version of the Web Audio API."
                  );
                }
              } else {
    ...
    */
          ...
          tsw.connect = function () {
            ...
            for (i = 0; i < number_of_arguments - 1; i++) {
              ...
              if (isObjectWithNode(first_arg) && isObjectWithNode(second_arg)) {
                if (
                  isNativeNode(first_arg.node) &&
                  isNativeNode(second_arg.node)
                ) {
                  connectNativeNodeToNativeNode(
                    first_arg.node,
                    second_arg.node,
                    second_arg.channel
                  );
                ...
                if (
                  isTswNode(first_arg.node) &&
                  isNativeNode(second_arg.node)
                ) {
                  connectTswNodeToNativeNode(
                    first_arg.node,
                    second_arg.node,
                    second_arg.channel
                  );
                ...
                if (
                  isNativeNode(first_arg.node) &&
                  isTswNode(second_arg.node)
                ) {
                  connectNativeNodeToTswNode(
                    first_arg.node,
                    second_arg.node,
                    second_arg.channel
                  );
           ...
           */
          tsw.disconnectAfterTime = function (
            nodeToDisconnect,
            timeToDisconnect
          ) {
            nodes_to_disconnect.push({
              node: nodeToDisconnect,
              time: timeToDisconnect,
            });
          };
          ...
          tsw.load = function () {
            ...
            var loadFile = function (
              ...
              var success = function () {
                ...
                that
                  .context()
                  .decodeAudioData(request.response, function (decodedBuffer) {
              ...
              var fail = function () {
                ...
                if (isFunction(failCallback)) {
                  ...
                  console.log(
                    "There was an error loading your file(s)",
                    request.status
                  );
                }
              };
          ...
          tsw.bufferBox = function (buff) {
            ...
            node.play = function (time, duration, detune) {
              ...
              sourceNode.onended = function () {
                if (
                  ...
                  !that.stopped &&
                  node.position() >=
                  node.buffer().length / tsw.context().sampleRate
                ) {
                  bufferPosition = 0;
                  that.stopped = true;
            ...
            node.pause = function (time) {
              ...
              if (!this.stopped) {
                bufferPosition = bufferPosition + tsw.now() - startTime;
              }
            };
            ...
            node.position = function (newPosition) {
              if (newPosition || newPosition === 0) {
                ...
                if (this.paused || this.stopped) {
                  ...
                  return bufferPosition + tsw.now() - startTime;
                }
              }
    ...
    5: [
      function (require, module, exports) {
        ...
        tsw.phaser = function (settings) {
          ...
          tsw.connect(
            node.input,
            allPassFilters[allPassFilters.length - 1],
            feedback,
            allPassFilters[0]
          );
    ...
    8: [
      function (require, module, exports) {
        ...
        tsw.chord = function (chord) {
          ...
          if (chord.isMinor) {
            notePositions.push(
              rootNotePosition + tsw.semitoneDifference("minor 3rd")
            );
          } else {
            notePositions.push(
              rootNotePosition + tsw.semitoneDifference("major 3rd")
            );
          }
          
          notePositions.push(
            rootNotePosition + tsw.semitoneDifference("perfect 5th")
          );
          notePositions.push(
            rootNotePosition + tsw.semitoneDifference("octave")
          );
        ...
        tsw.flat = function (note) {
          ...
          note = note.replace("#", "b");
          new_note = String.fromCharCode(
            note[0].toUpperCase().charCodeAt(0) + 1
          );
        ...
        tsw.sharp = function (note) {
          ...
          if (new_note === "B") {
            ...
            if (num_index > -1) {
              note =
                note.substring(0, num_index) +
                (note[num_index] - 1) +
                note.substring(num_index + 1);
            }
          }
        ...
        tsw.frequency = function (note) {
          var octave, keyNumber, note_index, note_without_octave;
